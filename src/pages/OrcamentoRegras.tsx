...
export default function OrcamentoRegras() {
  ...
  const aplicarRegras = async () => {
    ...
    try {
      ...
      // Build SQL-like update using the rules
      ...
      // Fetch all products
      const { data: produtos } = await supabase.from('orcamento_produtos').select('id, preco_unitario').gt('preco_unitario', 0);
  ...
  const getPreview = () => {
    const valMin = parseFloat(editValues[regras.find(r => r.campo === 'valor_minimo')?.id || ''] || '0') / 100;
    const valLoc = parseFloat(editValues[regras.find(r => r.campo === 'valor_locacao')?.id || ''] || '0') / 100;
    const valMinLoc = parseFloat(editValues[regras.find(r => r.campo === 'valor_minimo_locacao')?.id || ''] || '0') / 100;
    const valInst = parseFloat(editValues[regras.find(r => r.campo === 'valor_instalacao')?.id || ''] || '0') / 100;
  ...
  const preview = regras.length > 0 ? getPreview() : null;
    ...
    <Layout>
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Configure os percentuais usados para calcular automaticamente os preços dos produtos.</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Percent className="w-5 h-5" /> Percentuais</CardTitle>
            <CardDescription>Ao alterar o Valor Atual de um produto, os demais campos serão calculados com base nestas regras.</CardDescription>
          </CardHeader>
        ...
        {preview && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Simulação (Valor Atual = R$ {previewValue.toFixed(2)})</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <p className="text-muted-foreground">Valor Mínimo</p>
                  <p className="font-semibold">R$ {preview.valor_minimo}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Valor Locação</p>
                  <p className="font-semibold">R$ {preview.valor_locacao}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Valor Mín. Locação</p>
                  <p className="font-semibold">R$ {preview.valor_minimo_locacao}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Valor Instalação</p>
                  <p className="font-semibold">R$ {preview.valor_instalacao}</p>
                </div>
              </div>
            </CardContent>
          </Card>
