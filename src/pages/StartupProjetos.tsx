...
export default function StartupProjetos() {
  ...
  const requestedTab = searchParams.get('tab') || 'em-implantacao';
  const canAccessPciImplantacao = canAccess('implantacao/em-implantacao');
  const activeTab = !menuPermsLoading && requestedTab !== 'ppe' && !canAccessPciImplantacao ? 'ppe' : requestedTab;
  ...
  const fetchProjects = async () => {
    try {
      ...
      let projectsQuery = supabase
        .from('projects')
        .select('id, numero_projeto, cliente_condominio_nome, cliente_cidade, cliente_estado, vendedor_nome, created_at, updated_at, implantacao_status, implantacao_started_at, implantacao_completed_at, prazo_entrega_projeto, tipo_obra, tipo_implantacao, pendencia_status, pendencia_status_at')
      ...
      <div className="p-6 md:p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {(activeTab === 'em-implantacao' || activeTab === 'ppe') && (
            <Button variant="outline" onClick={() => navigate(`/implantacao-pendencias?tipo=${activeTab === 'ppe' ? 'PPE' : 'PCI'}`)}>
