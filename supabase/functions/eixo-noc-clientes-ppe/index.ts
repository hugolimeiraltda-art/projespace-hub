...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const expectedApiKey = Deno.env.get('PPE_CLIENTES_API_KEY') || Deno.env.get('CUSTOMER_API_KEY')

    if (!expectedApiKey) {
      return jsonResponse({ success: false, error: 'PPE customer API key is not configured' }, 500)
    }
    ...
    if (error) {
      console.error('Eixo NOC PPE customer query error:', error)
    ...
    console.error('Unexpected Eixo NOC PPE customer API error:', error)
